"""Contact sourcing: offline JSON fallback, or live HubSpot REST paging.
...
HUBSPOT_PROPERTIES = [
    ...
    "lifecyclestage", "hs_last_activity_date", "hubspot_owner_id", "createdate",
]

# canonical field -> accepted source key variants (checked in order)
FIELD_ALIASES = {
    "email": ["email", "Email", "Email Address", "email_address"],
    "firstname": ["firstname", "First Name", "first_name", "firstName"],
    "lastname": ["lastname", "Last Name", "last_name", "lastName"],
    "company": ["company", "Company Name", "company_name", "Company", "companyname"],
    "jobtitle": ["jobtitle", "Job Title", "job_title", "title"],
    "phone": ["phone", "Phone Number", "phone_number", "mobilephone", "Mobile Phone Number"],
    "lifecyclestage": ["lifecyclestage", "Lifecycle Stage", "lifecycle_stage"],
    "last_activity_date": [
        "last_activity_date", "Last Activity Date", "notes_last_activity_date",
        "hs_last_activity_date",
    ],
    "owner": ["owner", "Owner", "Contact Owner", "hubspot_owner_id", "owner_id"],
    "createdate": ["createdate", "Create Date", "created_at", "createDate"],
}
...
def normalize_contact(raw: dict) -> dict:
    """Map an arbitrary HubSpot-export-shaped dict to canonical fields.

    Handles both a flat property dict and a HubSpot API object of the
    form {"id": ..., "properties": {...}}.
    """
    source = raw.get("properties") if isinstance(raw.get("properties"), dict) else raw
    canonical: dict = {}
    for field, aliases in FIELD_ALIASES.items():
        ...
        for alias in aliases:
            if alias in source and source[alias] not in (None, ""):
                value = source[alias]
                break
        canonical[field] = str(value) if value is not None else None
    return canonical


def load_json(path: str) -> list[dict]:
    """Load a JSON file of contacts (list of dicts, or {"results": [...]})."""
...
def fetch_hubspot(token: str | None) -> list[dict]:
    """Paginate GET /crm/v3/objects/contacts and return normalized contacts."""
    if not token:
        raise RuntimeError(
            "GTMOS_HUBSPOT_TOKEN is not set. Export a HubSpot private-app token "
            "or pass --input <file.json> to audit an offline export instead."
        )
    ...
    while True:
        ...
        contacts.extend(normalize_contact(item) for item in payload.get("results", []))

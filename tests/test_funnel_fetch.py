"""Offline tests for gtmos.funnel.fetch's retry/backoff layer
...
from gtmos.funnel import fetch
...
def test_request_with_retry_401_raises_immediately_no_token_leak(monkeypatch):
    token = "super-secret-hubspot-deal-token-value"
    calls = []
...
def test_fetch_hubspot_deals_paginates_through_request_with_retry(monkeypatch):
    page1 = {"results": [{"id": "1", "properties": {"dealname": "A"}}], "paging": {"next": {"after": "cursor1"}}}
    page2 = {"results": [{"id": "2", "properties": {"dealname": "B"}}], "paging": {}}
    ...
    deals = fetch.fetch_hubspot_deals("tok")

    assert len(deals) == 2
    assert deals[0]["dealname"] == "A"
    assert deals[1]["dealname"] == "B"
